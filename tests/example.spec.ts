...
import { test } from '../hooks/fixtures/go-go-url';
import { Page } from '../page-objects/page-factory/pages';
import { Key } from '../framework/helpers/key';
import { FooterCategory, FooterCompanyItem } from '../page-objects/menus/footer-menu-items';
...
test('Search page check', async ({ page }) => {
...
test('Shop iframe check', async ({ page }) => {
  const mainPage = await PageFactory.getPage(Page.Main);
  ...
  await mainPage.footerMenu.getMenuItem(FooterCategory.Company, FooterCompanyItem.Shop).click();
